...
def training(args: argparse.Namespace) -> None:
    ...
    if type(args.task_dataset) == list: # Multiple datasets
        each_train_dataset = [CaptioningDataset(os.path.join(args.preprocess_path, each_dataset, f'train_data.pkl')) for each_dataset in args.task_dataset]
        each_valid_dataset = [CaptioningDataset(os.path.join(args.preprocess_path, each_dataset, f'valid_data.pkl')) for each_dataset in args.task_dataset]
        ...
        dataset_dict['train'] = CaptioningDataset(os.path.join(args.preprocess_path, args.task_dataset, f'train_data.pkl'))
        dataset_dict['valid'] = CaptioningDataset(os.path.join(args.preprocess_path, args.task_dataset, f'valid_data.pkl'))
    ...
    # We don't need to define loss function - they are defined in the model itself
    seq_loss = None
    ...
    for epoch_idx in range(start_epoch, args.num_epochs):
        ...
        for iter_idx, data_dicts in enumerate(tqdm(dataloader_dict['train'], total=len(dataloader_dict['train']), desc=f'Training - Epoch [{epoch_idx}/{args.num_epochs}]')):
            ...
            # Train - Forward pass
            outputs = model(images, captions, domain_ids)
